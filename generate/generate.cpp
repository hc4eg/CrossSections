...
int main(int argc, char *argv[])
{
	...
	// so it means max_eng = 42.MeV and min_eng = 18.MeV. Used for ke_p, ke_q will in a smaller range since ke_q = 60.0-(ke_p+me)-me-ke_recoil.
	// Q: so why not use e_p, e_q range from 18 to 42 MeV to make it symmetric?
	double max_t_theta = 15.*deg;
	// Q: Is the convention theta from -15deg to 15deg or 0 to 15deg
	...
	// Q: What's the relation between t_theta,t_phi and polar? See "Note" above.
	// Q: Why is min_polar? Lower polar angle no physics?
	...
	while(total_count < events_to_do)
		...
		do	{
			// Q: What is the reason to generate th_p from this random generating porcess?
			// ie: cos(th_p) uniformly random from 1 to 1-max_cos_polar
			// or from cos(max_t_theta) to 1
			...
			t_phi_p = atan(yy/zz);
		// Q: Why just use atan? this will only generate angle from -pi/2 to pi/2 and lose information of quadrant.
		// A: It seems only use t_theta_p and t_phi_p to check max_t_theta and max_t_phi condition. THEY'RE NOT SPHERICAL ANGLES
		...
		// (A: .dat file use t_theta_p than th_p)
